...
class replay():
    ...
    def pull_memory(self,state,action,reward,next_state,done):
        mask = ( done != 2 )
        if mask.sum() == 0:
            return
        else:
            ...
            for i in range(state.size(0)):
                if reward[i] < 0:
                    self.dead.appendleft([state[i:i+1],action[i:i+1],reward[i:i+1],next_state[i:i+1],done[i:i+1]])
                elif reward[i] > 0.1:
                    self.good.appendleft([state[i:i+1],action[i:i+1],reward[i:i+1],next_state[i:i+1],done[i:i+1]])
                else:
                    self.normal.appendleft([state[i:i+1],action[i:i+1],reward[i:i+1],next_state[i:i+1],done[i:i+1]])
