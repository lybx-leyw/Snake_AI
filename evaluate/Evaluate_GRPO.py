...
def evaluate_show(noise=0.0,path="snake_grpo_eva.pkl"):

    model = q_net.q_network()
    ...
    Game = env.game_env(paras=1,printLOG=True,logNAME="logs_grpo_eva.txt")
    while True:
        lastTime = time.time()
        direction = 0
        while True:
...
def evaluate_calc(path="snake_grpo_eva.pkl",index=0,Show=True):
...
def evaluate_grpo():
    ...
    if int(input("输入1计算模型性能:")) == 1:
        evaluate_calc(path="best\\snake_grpo.pkl",index=117,Show=True)

    if int(input("输入1演示模型性能:")) == 1:
        evaluate_show(noise=0.0,path="best\\snake_grpo.pkl")   
    print("评估完毕")     
